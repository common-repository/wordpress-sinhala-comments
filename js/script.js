...
$(document).ready(function() {
	...
	$("#comment").before('<marquee class="sinhala_comment_info" onmouseover=this.stop(); onmouseout=this.start(); scrollamount="3" scrolldelay="8">සිංහලෙන් comment දමන්න අවශ්‍යනම් පහල button එක click කලාම එන box එකේ type කරන්න. සිංහල Unicode වලින් comment box එකට වචනය add වේවි.<a href="https://webexplorar.com/wordpress-sinhala-comments-plugin/" target="_blank" class="sinhala-more_info" style="color:#990000;	cursor:help;">වැඩි විස්තර ඕනනම් මෙතන click කරන්න.</a></marquee><input style="margin: 5px 0 10px" type="button" class="sinhala_comment_button btn btn-sm btn-primary btn-primary-outline" value="සිංහලෙන් comments"/><br/><div style="display:none;" class="sinhala_comment_wrapper">Singlish Box<textarea  style="margin-top: 7px;" size="22" name="sinhalaCommentBox" id="sinhalaCommentBox"></textarea></div>');



	//*************** Show Comment Box *******************************

	$('.sinhala_comment_button').click(function () {
		$('.sinhala_comment_wrapper').slideToggle(300);
		$("#sinhalaCommentBox").focus();		
	});


	//*************** Convert Process *******************************

	$("#sinhalaCommentBox").keydown(
		function(){

			var a,b,c;		
			
			var char = $('#sinhalaCommentBox').val();  
			//********** special consonents *************
		...
		//$('#sinhalaCommentBox').val('');
			 $("#sinhalaCommentBox").keyup(function(e){
